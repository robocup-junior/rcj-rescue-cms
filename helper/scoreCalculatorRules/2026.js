    ...
    try {
            ...
            for (let j = 0; j < tile.scoredItems.length; j++) {
                switch (tile.scoredItems[j].item) {
                    case 'checkpoint': {
                        ...
                        score +=
                            Math.max(
                                tileCount * (5 - 2 * run.LoPs[checkPointCount]),
                                0
        ...
        if (run.rescueOrder) {
            ...
            for (let victim of run.rescueOrder) {
                if (victim.victimType == 'LIVE' && victim.zoneType == 'RED')
                    continue;
                if (victim.victimType == 'DEAD' && victim.zoneType == 'GREEN')
                    continue;
                if (
                    victim.victimType == 'DEAD' &&
                    liveCount != run.map.victims.live
                )
                    continue;
...
module.exports.calculateMazeScore = function (run) {
    ...
    for (let i = 0; i < run.tiles.length; i++) {
        ...
        const getVictimMaxKits = (victimType, cell, side) => {
            ...
            if (victimType === 'Cognitive') {
                if (
                    ...
                    !cell.tile.cognitiveTargets[side] ||
                    !cell.tile.cognitiveTargets[side].rings
                )
                    return 0;
        ...
        if (mapTiles[coord].tile.blue) {
            ...
            if (blueVisits > 0) {
                score += Math.max(
                    0,
                    MAX_BLUE_BONUS - blueVisits * BLUE_VISIT_PENALTY
                );
                blueTilesVisited++;
            }
        }
        ...
        if (mapTiles[coord].tile.victims.top !== 'None') {
            if (tile.scoredItems.victims.top) {
                const kitsAdded = addRescueKitsFor(
                    'top',
                    mapTiles[coord].tile.victims.top,
                    tile.scoredItems.rescueKits.top
                );
                let type = mapTiles[coord].tile.victims.top;
                if (type === 'Cognitive')
                    type += `:${getCognitiveStatus(mapTiles[coord], 'top')}`;
        ...
        if (mapTiles[coord].tile.victims.right !== 'None') {
            if (tile.scoredItems.victims.right) {
                ...
                if (type === 'Cognitive')
                    type += `:${getCognitiveStatus(mapTiles[coord], 'right')}`;
                addVictimCount(victims, type, kitsAdded);
        ...
        if (mapTiles[coord].tile.victims.bottom !== 'None') {
            if (tile.scoredItems.victims.bottom) {
                ...
                if (type === 'Cognitive')
                    type += `:${getCognitiveStatus(mapTiles[coord], 'bottom')}`;
                addVictimCount(victims, type, kitsAdded);
        ...
        if (mapTiles[coord].tile.victims.left !== 'None') {
            if (tile.scoredItems.victims.left) {
                const kitsAdded = addRescueKitsFor(
                    'left',
                    mapTiles[coord].tile.victims.left,
                    tile.scoredItems.rescueKits.left
                );
                let type = mapTiles[coord].tile.victims.left;
                if (type === 'Cognitive')
                    type += `:${getCognitiveStatus(mapTiles[coord], 'left')}`;
                addVictimCount(victims, type, kitsAdded);
    ...
    const reliabilityBonus =
        totalVictimCount * 10 +
        Math.min(rescueKits, MAX_RESCUE_KITS) * 10 +
        blueTilesVisited * 10 -
        run.LoPs * 15;
