...
function extractTargets(
    ...
    for (let i = 1; i <= maxY; i += 2) {
        // y (length)
        for (let j = 1; j <= maxX; j += 2) {
            // x (width)
            for (let k = 0; k <= maxZ; k++) {
                ...
                if (!cell || !cell.isTile || !cell.tile || !cell.tile.victims)
                    continue;
    ...
    for (const cell of map.cells) {
        ...
        for (const dir of directions) {
            ...
            if (
                ...
                ['PHI', 'PSI', 'OMEGA'].includes(victimType)
            ) {
                const key = `${cell.x},${cell.y},${cell.z},${dir}`;
                const victimLetter = victimNumberMap.get(key) || null;
    ...
    return targets.sort((a, b) => {
        ...
        // Final fallback
        return (a.colorCode || a.victimType).localeCompare(
            b.colorCode || b.victimType
        );
    });
}
...
function drawTarget(doc, x, y, target) {
    ...
    if (target.type === 'Cognitive') {
        ...
        imagePath = path.join(
            __dirname,
            '../public/images/cognitive_targets',
            `${colorCode}.png`
        );
        ...
        imagePath = path.join(
            __dirname,
            '../public/images/letter_victims',
            `${fileMap[victimType]}.png`
        );
    ...
    if (isExistFile(imagePath)) {
        ...
        doc.fontSize(10);
        doc.text(
            target.colorCode || target.victimType,
            x,
            y + TARGET_SIZE / 2 - 5,
            {
                width: TARGET_SIZE,
    ...
    if (victimLetter) {
        ...
        doc.circle(circleX, circleY, circleRadius).fill(circleColor);
 ...
 */
function generateCompetitionTargetsPDF(
    map,
    outputPath = null,
    includeLetterVictims = false,
    includeCognitiveTargets = true
    ...
    while (targetIndex < targets.length) {
        ...
        // Header
        doc.fontSize(16);
        const title = includeLetterVictims
            ? `Letter Victims / Cognitive Targets - ${map.name}`
            : `Cognitive Targets - ${map.name}`;
        ...
        doc.fontSize(10);
        doc.text(
            `Page ${Math.floor(targetIndex / (COLS * ROWS)) + 1}`,
            50,
            40,
            {
                width: 495,
...
function generateAndSendPDF(
    ...
    console.log(
        `Generating PDF: ${validPaperSize}, includeLetterVictims: ${includeLetterVictims}`
    );
    ...
    res.setHeader(
        'Content-Disposition',
        `inline; filename="targets.pdf"; filename*=UTF-8''${encodeURIComponent(`targets-${map.name}.pdf`)}`
    );
    ...
    while (targetIndex < targets.length) {
        ...
        const availableHeight =
            pageHeight - MARGIN_Y - headerHeight - bottomMargin;
        const cellHeight = availableHeight / ROWS;
 ...
 */
function generateAndSendBulkPDF(
    res,
    maps,
    competitionName,
    leagueName,
    ...
    res.setHeader(
        'Content-Disposition',
        `inline; filename="bulk-targets.pdf"; filename*=UTF-8''${encodeURIComponent(`targets-${competitionName}-${leagueName}.pdf`)}`
    );
