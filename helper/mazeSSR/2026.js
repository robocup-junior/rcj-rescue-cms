...
async function drawMazeCanvas(map) {
  ...
  for (let z = 0; z < numFloors; z++) {
    ...
    for (const cell of map.cells) {
      if (cell.z === z && cell.isTile && cell.tile && cell.tile.reachable) {
    ...
    if (map.startTile && map.startTile.z === z && (map.startTile.x !== 0 || map.startTile.y !== 0)) {
    ...
    const fRawWidth = getCoord(fRenderMaxX) - getCoord(fRenderMinX) + getSize(fRenderMaxX);
    const fRawHeight = getCoord(fRenderMaxY) - getCoord(fRenderMinY) + getSize(fRenderMaxY);
    ...
    totalCanvasHeight += fSectionHeight * SCALE_FACTOR + (z < numFloors - 1 ? 20 * SCALE_FACTOR : 0); 
  }
  ...
  for (let z = 0; z < numFloors; z++) {
    ...
    const rawWidth = bounds.rawWidth;
    const rawHeight = bounds.rawHeight;
    const labelHeight = bounds.labelHeight;

    const drawnSegments = new Set();
    const drawSegment = (x1, y1, x2, y2) => {
      // Normalize segment to avoid duplicates in different directions
      const segmentKey = x1 < x2 || (x1 === x2 && y1 < y2) 
        ? `${x1},${y1},${x2},${y2}` 
        : `${x2},${y2},${x1},${y1}`;
    ...
    for (const cell of map.cells) {
      ...
      if (cell.x < renderMinX || cell.x > renderMaxX || cell.y < renderMinY || cell.y > renderMaxY) continue;
      ...
      if (cell.isWall) {
        ...
        let t1, t2;
        if (cell.x % 2 === 0) { // vertical wall
          t1 = map.cells.find(c => c.x === cell.x - 1 && c.y === cell.y && c.z === z);
          t2 = map.cells.find(c => c.x === cell.x + 1 && c.y === cell.y && c.z === z);
        } else { // horizontal wall
          t1 = map.cells.find(c => c.x === cell.x && c.y === cell.y - 1 && c.z === z);
          t2 = map.cells.find(c => c.x === cell.x && c.y === cell.y + 1 && c.z === z);
        }
    ...
    for (let r = renderMinY + (renderMinY % 2 === 0 ? 0 : 1); r <= renderMaxY; r += 2) {
      for (let c = renderMinX + (renderMinX % 2 === 0 ? 0 : 1); c <= renderMaxX; c += 2) {
        // Only draw pillar if at least one adjacent tile is reachable
        const adjacentTiles = [
          map.cells.find(cell => cell.x === c - 1 && cell.y === r - 1 && cell.z === z),
          map.cells.find(cell => cell.x === c + 1 && cell.y === r - 1 && cell.z === z),
          map.cells.find(cell => cell.x === c - 1 && cell.y === r + 1 && cell.z === z),
          map.cells.find(cell => cell.x === c + 1 && cell.y === r + 1 && cell.z === z)
    ...
    if (z < numFloors - 1) {
      const lineY = currentY + bounds.rawHeight * SCALE_FACTOR + 10 * SCALE_FACTOR; // Exactly 10px below current map
      ctx.save();
