...
function getEntranceOrExit(map, tile) {
    ...
    const checkNeighbor = (nx, ny, nz) => {
        ...
        if (t && !isEvacTile(t)) {
            if (
                map.startTile2 &&
                t.x === map.startTile2.x &&
                t.y === map.startTile2.y &&
                t.z === map.startTile2.z
            )
                return 'Exit';
            return 'Entrance';
        }
...
async function drawLineCanvas(map) {
    ...
    for (let z = 0; z < numFloors; z++) {
        ...
        totalCanvasHeight +=
            fRawHeight * SCALE_FACTOR +
            (z < numFloors - 1 ? floorGap * SCALE_FACTOR : 0);
    }
    ...
    for (let z = 0; z < numFloors; z++) {
        ...
        for (const tile of tiles) {
            ...
            const isStartTile =
                map.startTile &&
                tile.x === map.startTile.x &&
                tile.y === map.startTile.y &&
                tile.z === map.startTile.z;
            ...
            if (
                map.startTile2 &&
                tile.x === map.startTile2.x &&
                tile.y === map.startTile2.y &&
                tile.z === map.startTile2.z
            ) {
            ...
            if (ev) {
                if (ev.status === 'Entrance')
                    await drawOverlay('ev-entrance.png', false, ev.rot);
                if (ev.status === 'Exit')
                    await drawOverlay('ev-exit.png', false, ev.rot);
            }
            ...
            if (tile.checkPoint || isStartTile || isScoring) {
                ...
                if (indexText) {
                    ...
                    ctx.fillRect(
                        TILE_SIZE / 2 - textWidth - 4,
                        -TILE_SIZE / 2 + 1,
                        textWidth + 2,
                        14
                    );
                    ctx.fillStyle =
                        tile.checkPoint || isStartTile ? 'orange' : '#0abde3';
                    ctx.textAlign = 'right';
                    ctx.fillText(
                        indexText,
                        TILE_SIZE / 2 - 3,
        ...
        if (z < numFloors - 1) {
            const lineY =
                currentY +
                bounds.rawHeight * SCALE_FACTOR +
                (floorGap / 2) * SCALE_FACTOR;
            ctx.save();
