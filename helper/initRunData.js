    ...
    if (map) {
        ...
        if (rule == '2025E' || rule == '2026E') {
            ...
            for (let index = run.tiles.length - 1; index >= 0; index--) {
                if (
                    run.tiles[index].scoredItems.some(
                        (item) => item.item == 'ramp'
                    )
                ) {
                    if (rampContinueFlag) {
                        run.tiles[index].scoredItems.splice(
                            run.tiles[index].scoredItems.findIndex(
                                (item) => item.item === 'ramp'
                            ),
                            1
                        );
        ...
        if (run.isNL) {
            ...
            run.rescueOrder = [];
            if (scored) {
                run.rescueOrder = run.rescueOrder.concat(
                    new Array(map.victims.live).fill({
                        victimType: 'LIVE',
                        zoneType: 'GREEN',
                    })
                );
                run.rescueOrder = run.rescueOrder.concat(
                    new Array(map.victims.dead).fill({
                        victimType: 'DEAD',
                        zoneType: 'RED',
                    })
                );
