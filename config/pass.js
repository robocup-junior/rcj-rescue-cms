...
exports.ensureAdminApi = function ensureAdminApi(req, res, next) {
    ...
    else res.status(400).send('You need to be admin to do this');
};
...
exports.ensureSuperApi = function ensureSuperApi(req, res, next) {
    if (req.user && req.user.superDuperAdmin === true) next();
    else res.status(400).send("You need to be 'superDuperAdmin' to do this");
};
